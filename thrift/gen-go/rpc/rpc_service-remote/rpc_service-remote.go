...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  string Run(i64 time, string message,  pairs)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  case "Run":
    if flag.NArg() - 1 != 3 {
      fmt.Fprintln(os.Stderr, "Run requires 3 args")
      flag.Usage()
    }
    ...
    containerStruct2 := rpc.NewRpcServiceRunArgs()
    err13 := containerStruct2.ReadField3(jsProt12)
    if err13 != nil {
      Usage()
      return
    }
    argvalue2 := containerStruct2.Pairs
    value2 := argvalue2
    fmt.Print(client.Run(context.Background(), value0, value1, value2))
    fmt.Print("\n")
    break
  case "":
    Usage()
